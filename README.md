...
O projeto utiliza tecnologias de última geração para garantir velocidade e conversão:

* React.js + Vite: Performance superior com renderização otimizada e carregamento instantâneo.
* Framer Motion: Animações de scroll premium e contadores numéricos dinâmicos para prova social.
* Google Analytics (GA4): Monitoramento de tráfego e conversões integrado (ID de Medição: `G-3DTRWPQC1E`).
* SEO Local: Arquivos `sitemap.xml` e `robots.txt` configurados para dominar as buscas orgânicas em Angola.
* WhatsApp Flutuante: Canal de conversão rápida integrado diretamente com a API do WhatsApp.
* Responsividade Total: Navegação adaptável com menu hambúrguer vertical otimizado para dispositivos móveis.
